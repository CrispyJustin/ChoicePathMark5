...
export function Settings() {
  ...
  // --- Handlers ---
  const handleImport = () => {
    if (!confirm(`Import ${toImport.length} student${toImport.length > 1 ? "s" : ""} from this device into your cloud board?`)) return;
          ...
          <>
            ...
            {user && (
              <section className="bg-card border-2 rounded-2xl p-5 space-y-3">
                ...
                <p className="text-sm text-muted-foreground">Signed in as <span className="font-semibold">{user.email}</span></p>

                {showImport && (
                  <div className="border rounded-xl p-4 bg-muted/40 space-y-2">
                    <p className="text-sm font-semibold">📥 {toImport.length} student{toImport.length > 1 ? "s" : ""} found on this device</p>
                    <button onClick={handleImport} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground font-bold text-sm">
                      Import Local Data to Cloud
                    </button>
                  </div>
                )}

                {importDone && <p className="text-sm text-green-600 font-semibold">✓ Local students imported successfully.</p>}

                <button onClick={handleLogout} className="px-4 py-2 rounded-lg bg-destructive text-destructive-foreground font-bold text-sm">
                  Sign out
                </button>
              </section>
            )}
            ...
            {user && (
              <section className="bg-card border-2 rounded-2xl p-5 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Share this board with another teacher or aide by email.</p>
                </div>
                ...
                {isCurrentBoardOwner ? (
                  <div className="flex gap-2 flex-wrap items-center">
                    ...
                    />
                    <button onClick={handleShareBoard} disabled={!shareEmail.trim() || Boolean((store as any).sharingBusy)} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground font-bold disabled:opacity-50">
                      Share Board
                    </button>
                  </div>
                ) : (
                  <p className="text-sm text-muted-foreground rounded-xl border-2 p-3 bg-muted/30">This board was shared with you. The board owner manages access.</p>
                )}

                {shareMessage && <p className="text-sm font-semibold text-muted-foreground">{shareMessage}</p>}
                {(store as any).sharingError && <p className="text-sm font-semibold text-destructive">{(store as any).sharingError}</p>}
                    ...
                    <ul className="space-y-2">
                      {((store as any).boardMembers ?? []).map((member: any) => (
                        <li key={member.id} className="flex items-center gap-2 justify-between rounded-xl border-2 p-3">
                          ...
                          {isCurrentBoardOwner && (
                            <button onClick={() => handleRemoveSharedEmail(member.id, member.member_email)} className="px-3 py-1.5 rounded-lg bg-destructive text-destructive-foreground text-sm font-bold">
                              Remove
                            </button>
                          )}
                        </li>
              ...
              <div className="flex gap-2">
                {([5, 8, 10] as const).map((n) => (
                  <button
                    ...
                    className={`px-6 py-3 rounded-xl font-bold border-2 ${store.pathLength === n ? "bg-primary text-primary-foreground border-primary" : "bg-card"}`}
                  >
                    {n} spaces
                  </button>
                ))}
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {THEMES.map((t) => (
                  <button
                    ...
                    className={`p-4 rounded-xl border-2 text-left ${store.theme === t.id ? "border-primary bg-primary/5" : "border-border"}`}
                  >
            ...
            <section className="bg-card border-2 rounded-2xl p-5">
              ...
              <div className="space-y-2 mb-4">
                {store.students.map((s) => (
                  <StudentRow key={s.id} student={s} onUpdate={store.updateStudent} onDelete={store.deleteStudent} />
                ))}
              </div>
              <div className="border-t pt-4">
                <h3 className="font-bold mb-2">Add Student</h3>
                <div className="flex gap-2 flex-wrap items-center">
                  <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="First name" className="px-3 py-2 rounded-lg border-2 flex-1 min-w-[140px]" />
                  <AvatarPicker value={newAvatar} onChange={setNewAvatar} />
                  <button onClick={() => { if (!newName.trim()) return; store.addStudent(newName.trim(), newAvatar); setNewName(""); }} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground font-bold">
                    Add
                  </button>
                </div>
              </div>
            ...
            <section className="bg-card border-2 rounded-2xl p-5 border-primary shadow-sm space-y-3">
              <h2 className="text-xl font-bold text-foreground flex items-center gap-2">⭐ ChoicePath Premium</h2>
              <p className="text-sm text-muted-foreground">Support ChoicePath development and remove advertisements.</p>
              <button onClick={() => alert("Stripe checkout module coming soon!")} className="px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm">
  ...
  return (
    <div className="flex gap-1 flex-wrap">
      {AVATAR_OPTIONS.map((a) => (
        <button key={a} onClick={() => onChange(a)} className={`text-2xl h-10 w-10 rounded-lg border-2 ${value === a ? "border-primary bg-primary/10" : "border-border"}`}>
          {a}
        </button>
      ))}
    </div>
...
function StudentRow({ student, onUpdate, onDelete }: { student: Student; onUpdate: (id: string, patch: Partial<Student>) => void; onDelete: (id: string) => void }) {
    ...
    return (
      <div className="p-3 rounded-xl border-2 bg-muted/30 space-y-2">
        <div className="flex gap-2 items-center flex-wrap">
          <input value={name} onChange={(e) => setName(e.target.value)} className="px-3 py-2 rounded-lg border-2 flex-1 min-w-[140px]" />
          <AvatarPicker value={avatar} onChange={setAvatar} />
        </div>
        <div className="flex gap-2">
          <button onClick={() => { onUpdate(student.id, { name: name.trim() || student.name, avatar }); setEditing(false); }} className="px-3 py-1.5 rounded-lg bg-primary text-primary-foreground font-bold text-sm">Save</button>
          <button onClick={() => setEditing(false)} className="px-3 py-1.5 rounded-lg bg-card border-2 text-sm font-bold">Cancel</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl border-2 flex-wrap">
      <span className="text-3xl">{student.avatar.startsWith("data:") ? "🖼️" : student.avatar}</span>
      <span className="font-bold flex-1 min-w-[80px]">{student.name}</span>
      <button onClick={() => setEditing(true)} className="px-3 py-1.5 rounded-lg bg-card border-2 text-sm font-bold">Edit</button>
      <button onClick={() => { if (confirm(`Delete ${student.name}?`)) onDelete(student.id); }} className="px-3 py-1.5 rounded-lg bg-destructive text-destructive-foreground text-sm font-bold">Delete</button>
    </div>
  );
}
